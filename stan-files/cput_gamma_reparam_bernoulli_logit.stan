...
functions {
  ...
  // define a function getEU that takes the magnitude of the option's
  ...
  real getEU(real mag_a, real prob_a, real mag_b, real prob_b) {
    real eu;
...
model {
  ...
  for (i in 1:(num_subjects)) {
       ...
       for (j in 1:(prospects_per_subj[i])) {
        ...
        // for all prospects
         UE1 = getEU(x1[i,j], p1[i,j], x2[i,j], not_p1[i,j]);
         UE2 = getEU(x4[i,j], p2[i,j], x3[i,j], not_p2[i,j]);
         ...
         chose_option1[i,j] ~ bernoulli_logit((UC1 - UC2));
  ...
  { // local section to save time and space
    ...
    for (i in 1:num_subjects) {
      ...
      for (j in 1:(prospects_per_subj[i])) {
        ...
        pred_choice[i,j] = bernoulli_logit_rng((UC1 - UC2));
