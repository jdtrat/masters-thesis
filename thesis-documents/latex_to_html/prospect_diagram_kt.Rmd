...
\tikzstyle{labels} = [text width=5em, text centered]
...
\tikzstyle{n2} = [shape=circle, draw, align=center, bottom color=white, top color=deepskyblue!40, thick]
    ...
    child { node[main](A1) {}
        child{ node[x1](X1) {$\$3000$}}
        child{ node[n1](N1) {$\$0$}}}
    child { node[main](A2) {}
        child{ node[n2](N2) {$\$0$}}
        child{ node[x2](X2) {$\$4000$}}};
    ...
    \node[labels] at (-2.5,-0.7) {\scriptsize Option 1 (\$3000, 1)};
    \node[labels] at (2.5,-0.7) {\scriptsize Option 2 (\$4000, 0.8)};
    \node[labels] at (-1,-2) {$(1 - 1.0)$};
    \node[labels] at (-3,-2) {$1.0$};
    \node[labels] at (0.95,-2) {$(1 - 0.8)$};
    \node[labels] at (3.075,-2) {$0.8$};
