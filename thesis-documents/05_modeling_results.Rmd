...
EUT provides a better explanation of the observed data on the sure bet or gamble task ($BF_{\frac{\rho, \tau}{\gamma, \tau}}$ = `r round(targets::tar_read(log_bayes_factor_ra_tau_over_tau_gamma)$bf_median_based, 2)`) and generalizes better than CPUT ($\text{LOOIC}_{\rho, \tau} - \text{LOOIC}_{\gamma, \tau}$ =  `r round(targets::tar_read(looic_ra_tau_over_tau_gamma), 2)`). The posterior predictive choice accuracy -- the percentage of choices simulated with the posterior estimate of each subject's parameters -- are similar, though better for EUT (`r targets::tar_read(pp_choice_accuracy_ra_tau)`) compared to CPUT (`r targets::tar_read(pp_choice_accuracy_tau_gamma)`). 
