...
Taken together, the model comparison metrics show that EUT provides a better explanation of both the observed data on the sure bet or gamble task ($BF_{\frac{\rho, \tau}{\gamma, \tau}}$ = `r round(targets::tar_read(log_bayes_factor_ra_tau_over_tau_gamma)$bf_median_based, 2)`) and generalizes better than CPUT ($\text{LOOIC}_{\rho, \tau} - \text{LOOIC}_{\gamma, \tau}$ =  `r round(targets::tar_read(looic_ra_tau_over_tau_gamma), 2)`). The posterior predictive choice accuracy -- the percentage of choices simulated with the posterior estimate of each subject's parameters -- are similar, though better for EUT (`r targets::tar_read(pp_choice_accuracy_ra_tau)`) compared to CPUT (`r targets::tar_read(pp_choice_accuracy_tau_gamma)`). 
...
Interestingly, the risk sensitivity term, $\rho$, from EUT with the counterfactual weighting from CPUT offers the best explanation for the observed human choice data ($BF_{\frac{\text{CPUT + Softmax + Risk Sensitivity}}{\rho, \tau}}$ = `r round(targets::tar_read(log_bayes_factor_ra_tau_gamma_over_ra_tau)$bf_median_based, 2)`). This does come with a cost of decreased generalizability ($\text{LOOIC}_{\rho, \tau} - \text{LOOIC}_\text{CPUT + Softmax + Risk Sensitivity}$ = `r round(targets::tar_read(looic_ra_tau_over_ra_tau_gamma), 2)`) and increased posterior predictive choice accuracy (`r targets::tar_read(pp_choice_accuracy_ra_tau_gamma)` relative to EUT's `r targets::tar_read(pp_choice_accuracy_ra_tau)`). 
