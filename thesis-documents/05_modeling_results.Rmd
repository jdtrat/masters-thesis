...
#| fig.scap = "Posterior distribution of $\\gamma$ estimated with choice data informed from Kahneman and Tversky (1979).",
...
EUT provides a better explanation of both the observed data on the sure bet or gamble task ($BF_{\frac{\rho, \tau}{\gamma, \tau}}$ = `r round(targets::tar_read(log_bayes_factor_ra_tau_over_tau_gamma)$bf_median_based, 2)`) and generalizes better than CPUT ($\text{LOOIC}_{\rho, \tau} - \text{LOOIC}_{\gamma, \tau}$ =  `r round(targets::tar_read(looic_ra_tau_over_tau_gamma), 2)`). The posterior predictive choice accuracy -- the percentage of choices simulated with the posterior estimate of each subject's parameters -- are similar, though better for EUT (`r targets::tar_read(pp_choice_accuracy_ra_tau)`) compared to CPUT (`r targets::tar_read(pp_choice_accuracy_tau_gamma)`). 

Further, the group-level posterior parameter distribution for $\tau$, the common variable to these two models, is higher for EUT than CPUT (95% HDI for EUT is `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_ra_tau", "mu_tau")`; CPUT = `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_tau_gamma", "mu_tau")`). This suggests more utility maximizing decisions for EUT relative to more random choice behavior for CPUT.

### CPUT + Risk Sensitivity versus EUT

Adding the risk sensitivity term, $\rho$, from EUT to CPUT offers the best explanation for the observed human choice data ($BF_{\frac{\rho, \tau, \gamma}{\rho, \tau}}$ = `r round(targets::tar_read(log_bayes_factor_ra_tau_gamma_over_ra_tau)$bf_median_based, 2)`). This does come with a cost of decreased generalizability ($\text{LOOIC}_{\rho, \tau} - \text{LOOIC}_{\rho,\tau,\gamma}$ = `r round(targets::tar_read(looic_ra_tau_over_ra_tau_gamma), 2)`) but increased posterior predictive choice accuracy (`r targets::tar_read(pp_choice_accuracy_ra_tau_gamma)` relative to EUT's `r targets::tar_read(pp_choice_accuracy_ra_tau)`). 

Further, the group-level posterior parameter distribution for $\tau$ depict similar sensitivities to utility differences (95% HDI for 'CPUT + Softmax + Risk Sensitivity' = `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_ra_tau_gamma", "mu_tau")`; 'EUT + Softmax = `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_ra_tau", "mu_tau")`) and risk sensitivity (95% HDI for 'CPUT + Softmax + Risk Sensitivity' = `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_ra_tau_gamma", "mu_rho")`; 'EUT + Softmax' = `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_ra_tau", "mu_rho")`).

### CPUT + Risk Sensitivity versus CPUT

Incorporating risk sensitivity to CPUT results in a better explanation for the observed data ($BF_{\frac{\rho, \gamma, \tau}{\gamma, \tau}}$ = `r round(targets::tar_read(log_bayes_factor_ra_tau_gamma_over_tau_gamma)$bf_median_based, 2)`), is more generalizable, ($\text{LOOIC}_{\rho, \gamma, \tau} - \text{LOOIC}_{\gamma, \tau}$ =  `r round(targets::tar_read(looic_ra_tau_gamma_over_tau_gamma), 2)`), and has a higher posterior predictive choice accuracy (`r targets::tar_read(pp_choice_accuracy_ra_tau_gamma)`) relative to CPUT's (`r targets::tar_read(pp_choice_accuracy_tau_gamma)`). 

Interestingly, the group-level posterior parameter distribution for $\tau$ with the CPUT + Risk Sensitivity more closely reflects that of EUT. At the same time, the group-level posterior parameter distribution for $\gamma$ is much more tightly concentrated towards zero than for CPUT (95% HDI for CPUT + Risk Sensitivity is `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_ra_tau_gamma", "mu_gamma", digits = 4, fancy_label = TRUE)`; CPUT = `r get_hdi_dist(targets::tar_read(group_level_hdi), "sborg_tau_gamma", "mu_gamma", digits = 4, fancy_label = TRUE)`). This suggests more a lower weighting on counterfactual information while accounting for risk preferences. 
